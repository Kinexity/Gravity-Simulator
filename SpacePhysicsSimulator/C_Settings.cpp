...
bool operator!=(S_Settings_Storage set_1, S_Settings_Storage set_2) {
	return
		...
		(set_1.calculate_energy_error != set_2.calculate_energy_error) ||
...
C_Settings::C_Settings(PCL::C_Event_Log& ev_log_obj)
	...
	std::unique_lock<std::mutex> guard(synchronizer);
	settings_file.open("settings.bin", std::ios::out | std::ios::in | std::ios::binary);
	if (!settings_file.is_open()) {
		event_log_obj() << "Settings file doesn't exist! Creating new one..." << _endl_;
		settings_file.open("settings.bin", std::ios::out);
		settings_file.close();
		settings_file.open("settings.bin", std::ios::out | std::ios::in | std::ios::binary);
		...
		}
		{
			...
			case 2: {
				do {
					...
					std::cin >> settings.num_of_threads_override;
				} while (input_error());
				break;
			}
			case 3: {
				do {
					...
					std::cin >> settings.ram_GiB_limit;
				} while (input_error());
				break;
			}
			case 4: {
				do {
					...
					std::cout << "1 - Uzywaj" << '\n';
					std::cout << "Twoj wybor: ";
					std::cin >> settings.enable_gpu_acceleration;
				} while (input_error());
				break;
			}
			case 5: {
				do {
					...
					std::cout << "1 - Uzywaj" << '\n';
					std::cout << "Twoj wybor: ";
					std::cin >> settings.full_cuda_acceleration;
				} while (input_error());
				break;
			}
			case 6: {
				do {
					...
					std::cout << "1 - Uzywaj" << '\n';
					std::cout << "Twoj wybor: ";
					std::cin >> settings.calculate_energy_error;
				} while (input_error());
				break;
			}
			case 7: {
				do {
					...
					std::cout << "Dane: ";
					std::cin >> settings.test_sim_basic_data().num_of_objects >> settings.test_sim_basic_data().sim_duration >> settings.test_sim_basic_data().cycles_per_second_exponent;
